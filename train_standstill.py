...
# DEBUG: Global counter for first frame detection
...
class ZbotWalkingTaskConfig(ksim.PPOConfig):
    ...
    slow_motion: bool = xax.field(
        value=False,
        help="If True, renders in slow motion (0.25x speed). Use with run_mode=view",
    )
...
class ZbotWalkingTask(ksim.PPOTask[ZbotWalkingTaskConfig]):
    ...
    def __init__(self, config: ZbotWalkingTaskConfig):
        ...
        # Set render_slowdown based on slow_motion flag
        if config.slow_motion:
            object.__setattr__(config, "render_slowdown", 0.25)
    ...
    def get_rewards(self, physics_model: ksim.PhysicsModel) -> list[ksim.Reward]:
        """Get rewards - simplified to only reward staying alive and staying upright.

        This training configuration rewards the robot for:
        1. Staying alive (not falling or terminating)
        2. Staying upright (maintaining vertical orientation)
        """
        rewards = [
...
if __name__ == "__main__":
    ZbotWalkingTask.launch(
        ZbotWalkingTaskConfig(
            ...
            # DEBUG: Set to zero to eliminate simulated action delay
